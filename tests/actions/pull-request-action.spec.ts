...
describe('Test Action PullRequestAction', () => {
  ...
  test('test not execute as event is unknown', async () => {
    ...
    let receivedPayload: WebhookPayloadPullRequest | undefined = undefined;

    const fooMock: any = { dummyCall: jest.fn() };
    pullRequestAction.registerCallback(['unknown-event'], async (payload: WebhookPayloadPullRequest) => {
      fooMock.dummyCall();
